...
func NewSharding(ctx context.Context, shards int, logger log.Logger, stats *MapperStats) (ASTMapper, error) {
	shardSummer, err := newQueryShardSummer(ctx, shards, VectorSquasher, logger, stats)
	if err != nil {
		return nil, err
	}
...
func newQueryShardSummer(ctx context.Context, shards int, squasher Squasher, logger log.Logger, stats *MapperStats) (ASTMapper, error) {
	...
	return NewASTExprMapper(NewShardSummer(ctx, shards, squasher, logger, stats, newQueryShardLabeller(shards))), nil
}
...
func NewShardSummer(ctx context.Context, shards int, squasher Squasher, logger log.Logger, stats *MapperStats, shardLabeller ShardLabeller) *shardSummer {
	return &shardSummer{
		ctx: ctx,
