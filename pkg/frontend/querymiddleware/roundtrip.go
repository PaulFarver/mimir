...
func newQueryTripperware(
	...
	var partitionOffsetsReader *ingest.PartitionOffsetsReader
	if cfg.IngestStorageConfig.Enabled {
		...
		kafkaMetrics := ingest.NewKafkaReadClientMetrics("cortex_query_frontend_ingest_storage", registerer)
		kafkaClient, err := ingest.NewKafkaReadClient(cfg.IngestStorageConfig.KafkaConfig, kafkaMetrics, log)
		if err != nil {
			return nil, err
		}

		// TODO wrap the registerer adding "component", otherwise metrics have conflicts in single-binary mode
		partitionOffsetsReader = ingest.NewPartitionOffsetsReader(kafkaClient, cfg.IngestStorageConfig.KafkaConfig.Topic, cfg.IngestStorageConfig.KafkaConfig.LastProducedOffsetPollInterval, registerer, log)
