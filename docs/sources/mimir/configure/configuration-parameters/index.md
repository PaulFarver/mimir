...
bucket_store:
  ...
  # (advanced) Increasing the series preference results in fetching more series
  # to fulfil a Series request and reduces the volume of postings fetches.
  # Reducing the preference results in the opposite. Increase to reduce the rate
  # of fetched series bytes (see "Mimir / Queries" dashboard) or API calls to
  # the object store. Must be a positive floating point number.
